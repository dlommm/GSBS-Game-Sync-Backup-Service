...
- **`GSBS_TRUST_PROXY` caveat:** the value is the number of trusted reverse proxies in front of the server (`1` for the usual single proxy; unset or `0` disables header trust). The client IP is read from `X-Forwarded-For` counting from the **right**, since a proxy appends the peer it actually saw — entries to the left are client-written and never trusted. Set it no higher than the number of proxies you actually control, and never enable it when the server is directly reachable.
