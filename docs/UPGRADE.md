...
| Version | Change |
|---------|--------|
| **4.0.0** | `GSBS_SESSION_SECRET` must be 32+ characters and not a placeholder — **the server refuses to start otherwise**. Replace weak secrets before upgrading (`openssl rand -base64 32`; rotating logs out WebUI sessions, API clients unaffected). `GSBS_INSECURE_DEV_SECRET=1` bypasses for local dev. The `.deb` no longer depends on `libayatana-appindicator3`. **Storage quotas now count version history** and are enforced atomically — dashboard usage will appear higher (grandfathered: over-quota users can still shrink/replace). History tables are pruned by default (`GSBS_*_RETENTION_DAYS`, 0 = keep forever). |
