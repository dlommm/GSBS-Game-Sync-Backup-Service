...
- **PCGW sync schedule**: the direct-API sync source is **retired** (PCGamingWiki removed the `runcargoqueries` right from anonymous users, so the Phase 1 catalog scan returns `permissiondenied`). `pcgw_sync_source=api` is normalized to `s3` and migrated in the database on upgrade; only the bundle fetch is scheduled. The `pcgw-sync` maintainer tool depends on the same Cargo queries and is affected too.
