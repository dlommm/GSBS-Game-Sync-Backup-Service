...
| **PCGW API** — **RETIRED** | `pcgw_sync_source=api` (normalized to `s3`) | none | none |
...
|----------|---------|-------------|
| `GSBS_PCGW_SYNC_SOURCE` | (from DB) | `s3`. The retired `api` and legacy `github` values are accepted and normalized to `s3`. Overrides admin Settings when set. |
